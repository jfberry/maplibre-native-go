...
// Held for the lifetime of the render session; mbgl retains its own
...
func (m *Map) AttachMetalTexture(width, height uint32, scaleFactor float64) (*RenderSession, error) {
...
// device must remain valid for the lifetime of the render session;
...
func (m *Map) AttachMetalTextureWithDevice(device unsafe.Pointer, width, height uint32, scaleFactor float64) (*RenderSession, error) {
	if m == nil {
	...
	trackForLeak(s, "RenderSession (Metal)", func() bool { return s.ptr != nil })
	return s, nil
}
